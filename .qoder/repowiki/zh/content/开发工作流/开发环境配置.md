...
- [docs/tauri-versions.md](file://docs/tauri-versions.md)
</cite>

## 更新摘要
**所做更改**
- 更新了 README.md 简化后的项目标识信息
- 基于新的 AGENTS.md 和 CLAUDE.md 文档更新了开发命令说明
- 补充了新的前端架构文档和版本信息文档
- 更新了 IDE 配置指南，基于新的文档要求
- 完善了开发环境配置的最新状态
...
## 简介
本指南面向 Tauri + Vue + TypeScript 项目的开发者，提供从工具链安装到开发调试的完整配置说明。随着项目文档的演进，README.md 已简化为项目标识，详细的开发环境配置已在新的综合架构文档中得到完善。重点涵盖以下内容：
...
**更新** 基于新的 AGENTS.md 和 CLAUDE.md 文档，更新了开发命令和 IDE 配置指南
...
**更新** 基于新的 AGENTS.md 和 CLAUDE.md 文档，确认了开发命令和配置的准确性
...
- pnpm tauri dev：启动 Tauri 开发流程，内部会先执行 beforeDevCommand（即 pnpm dev），再启动 Tauri 应用，使前端与后端联调更便捷。
- 使用建议：
  - 快速修改前端样式与逻辑：使用 pnpm dev
  - 需要调用 Rust 命令或测试桌面功能：使用 pnpm tauri dev

**更新** 基于 AGENTS.md 和 CLAUDE.md 的开发命令说明，确认了命令的准确性和使用场景

**章节来源**
- [package.json:6-11](file://package.json#L6-L11)
- [src-tauri/tauri.conf.json:6-11](file://src-tauri/tauri.conf.json#L6-L11)
- [AGENTS.md:13-17](file://AGENTS.md#L13-L17)
- [CLAUDE.md:13-17](file://CLAUDE.md#L13-L17)
...
- 路径别名：配置 @ 路径别名指向 src 目录，支持 TypeScript 路径映射。
...
ClearScreen --> PathAlias["配置 @ 路径别名"]
PathAlias --> Ready(["开发就绪"])
```

**图表来源**
- [vite.config.ts:4-39](file://vite.config.ts#L4-L39)
...
- TypeScript Take Over 模式：若需在 .vue 文件中获得更精确的类型推断，可按 AGENTS.md 步骤禁用默认 TS 扩展并重载窗口。
- 建议在工作区设置中启用 Volar 的 Take Over 模式以提升类型体验。

**更新** 基于 AGENTS.md 的 IDE 配置指南，确认了插件组合和设置要求

**章节来源**
- [AGENTS.md:107-111](file://AGENTS.md#L107-L111)
- [README.md:5-17](file://README.md#L5-L17)

### 环境变量、路径别名与 TypeScript 类型定义
- 环境变量：TAURI_DEV_HOST 用于控制 HMR 的 WebSocket 主机与端口，未设置时禁用 HMR。
- 路径别名：配置 @ 路径别名指向 src 目录，在 tsconfig.json 中通过 baseUrl 和 paths 实现。
- TypeScript 类型定义：
...
**更新** 基于新的 tsconfig.json 配置，确认了路径别名的正确设置
...
- .vue 类型问题
  ...
  - 处理：启用 Volar Take Over 模式或按 AGENTS.md 步骤操作
  - 参考：[AGENTS.md:107-111](file://AGENTS.md#L107-L111)
...
## 结论
本指南提供了 Tauri + Vue + TypeScript 项目的完整开发环境配置方案。通过合理设置 Vite 端口与 HMR、明确 pnpm dev 与 pnpm tauri dev 的职责边界、规范 Rust 依赖与 Tauri CLI 工具链，以及优化 TypeScript 与 IDE 配置，可以显著提升开发效率与调试体验。随着项目文档的演进，建议开发者参考最新的 AGENTS.md、CLAUDE.md 和前端架构文档，确保遵循团队的最新开发约定。

**更新** 基于新的文档结构，强调了综合架构文档的重要性
...
- 新文档参考
  - AGENTS.md：AI 编码助手指南，包含详细的开发命令和代码风格
  - CLAUDE.md：Claude Code 开发指南，提供高阶架构说明
  - docs/frontend-architecture.md：前端架构设计文档
  - docs/tauri-versions.md：Tauri 版本信息文档