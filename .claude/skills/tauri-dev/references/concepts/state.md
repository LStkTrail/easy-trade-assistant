...
本文档详细讲解 Tauri 中的状态管理机制。

## 概述

Tauri 提供了类型安全的状态管理系统，允许在 Rust 后端的命令之间共享数据，而不需要使用全局变量。
...
// 可以在多个地方克隆 Arc
#[command]
fn clone_state(state: State<SharedState>) {
    let data_clone = state.data.clone();
    // 在另一个线程中使用
    std::thread::spawn(move || {
        let mut data = data_clone.lock().unwrap();
        data.push("from thread".to_string());
    });
}
```
...
#[command]
fn add_post(state: State<AppState>, post: Post) {
    let mut posts = state.db.posts.lock().unwrap();
    posts.push(post);
}
```
